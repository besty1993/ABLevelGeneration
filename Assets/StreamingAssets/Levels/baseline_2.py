...
def make_structure(absolute_ground, center_point, max_width, max_height):
    ...
    for row in reversed(total_tree):
        for item in row:
            print("sa",[item[0],item[1],round((((blocks[str(item[0])][1])/2)+ground),10)])
            complete_locations.append([item[0],item[1],round((((blocks[str(item[0])][1])/2)+ground),10)])
        ground = ground + (blocks[str(item[0])][1])
    ...
    for platform_set in final_platforms:
        ...
        for platform_set2 in final_platforms:
            if platform_set2 != platform_set:
                for i in platform_set2:
                    if i[0]+platform_size[0] > platform_set[0][0] and i[0]-platform_size[0] < platform_set[-1][0] \
                            and i[1] > platform_set[0][1]:
...
def create_simple_platform_structures(final_platforms, platform_centers, complete_locations, final_pig_positions):
    current_platform = 0
    for platform_set in final_platforms:
        platform_set_width = len(platform_set) * platform_size[0]
...
while (checker != ""):
    if checker == "\n":
        ...
        for current_level in range(number_levels):
            ...
            if (select_generator == 0) :
                complete_locations, final_pig_positions = create_simple_platform_structures(final_platforms, platform_centers,
                                                                                     complete_locations,
                                                                                     final_pig_positions)
            else :
            ...
            possible_trihole_positions, possible_tri_positions, possible_cir_positions, possible_cirsmall_positions \
                = find_additional_block_positions(complete_locations)
