...
def make_simple_structure(absolute_ground, center_point, max_width):
    total_tree = []  # all blocks of structure (so far)
...
def create_platforms(number_platforms, complete_locations, final_pig_positions):
    ...
    while len(final_platforms) < number_platforms:
        platform_width = randint(2, 7)
        if (simple_level) : platform_width = randint(2,4)
...
def create_platform_structures(final_platforms, platform_centers, complete_locations, final_pig_positions):
    ...
    for platform_set in final_platforms:
        ...
        if (simple_level) :
            complete_locations2, final_pig_positions2, final_TNT_positions = make_simple_structure(absolute_ground,
                                                                                                   center_point,
                                                                                                   max_width)
        else :
            complete_locations2, final_pig_positions2 = make_structure(absolute_ground, center_point, max_width, max_height)
...
def write_level_xml(complete_locations, selected_other, final_pig_positions, final_TNT_positions, final_platforms, number_birds, current_level, restricted_combinations):
    ...
    f = open(os.path.realpath(__file__)+"/../subset-%s.xml" % current_level, "w")
...
FILE = open(os.path.realpath(__file__)+"/../parameters.txt", 'r')
checker = FILE.readline()
finished_levels = 0
while (checker != ""):
    if checker == "\n":
        checker = FILE.readline()
        print(finished_levels)
    else:
        number_levels = int(deepcopy(checker))
        number_levels = number_levels*5                 # the number of levels to generate
        ...
        useless_parameter = FILE.readline()
        checker = FILE.readline()
        ...
        for current_level in range(number_levels):
            ...
            number_pigs = randint(0,2)  # number of pigs (if set too large then can cause program to infinitely loop)
